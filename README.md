...
API поднимется на `http://localhost:5280` (документация на `/swagger`), миграции всех модулей применяются автоматически при старте. PostgreSQL: `localhost:5435` (`viridisca` / `postgres` / `postgres`).
...
- Backend реализован как модульный монолит: 5 реализованных модулей (Identity/Academic/Grading/Curriculum/Scheduler), каждый со своими Domain/Application/Infrastructure/Presentation и собственной PostgreSQL-схемой; ещё несколько модулей (Analytics/Attendance/Communication/Finance) пока не реализованы
- Cross-module ссылки: обычные индексированные `Guid`-колонки без FK-констрейнта между схемами (например, `Curriculum.CourseInstance.TeacherUid` → `Academic`), а не project reference между модулями, поэтому модули остаются независимо компилируемыми

...
namespace Viridisca.Modules.Academic.Application.Students.Queries.GetStudent
{
    internal sealed class GetStudentQueryHandler : IRequestHandler<GetStudentQuery, StudentDto>
    {
        ...
        public async Task<StudentDto> Handle(GetStudentQuery request, CancellationToken cancellationToken)
        {
            ...
            var parentDtos = new List<StudentParentDto>();
            foreach (var parent in student.Parents)
            {
                parentDtos.Add(new StudentParentDto
                {
                    Uid = parent.Uid,
                    ParentUserUid = parent.ParentUserUid,
                    ParentFullName = await _userInfoService.GetUserFullNameAsync(parent.ParentUserUid, cancellationToken),
                    Relation = parent.Relation.ToString(),
                    IsPrimaryContact = parent.IsPrimaryContact,
                    HasLegalGuardianship = parent.HasLegalGuardianship,
                    Email = await _userInfoService.GetUserEmailAsync(parent.ParentUserUid, cancellationToken),
                    PhoneNumber = await _userInfoService.GetUserPhoneAsync(parent.ParentUserUid, cancellationToken)
                });
            }
